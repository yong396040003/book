...
public class BookPageActivity extends AppCompatActivity implements BookPageAdapter.MyOnScrollListener, View.OnClickListener {
    ...
    //上一章
    private List<BookPage> leftbookPageList;
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(final Message msg) {
            if (msg.obj != null && msg.arg1 == 1) {
                if (isLeft) {
                    leftbookPageList = new ArrayList<>();
                }
                ...
                try {
                    ...
                    if (!isStop) {
                        while (!"".equals(bodys)) {
                            ...
                            if (!isLeft) {
                                if (bookPageList.size() > 0) {
                                    if (bookPageList.get(bookPageList.size() - 1) == null) {
                                        bookPageList.remove(bookPageList.size() - 1);
                                    }
                                }
                                bookPageList.add(bookPage);
                            } else {
                                leftbookPageList.add(bookPage);
                            }
                        ...
                        if (pagerAdapter != null) {
                            //如果最第一页不为null 那么就让他变为null
                            if (count > 1 && isLeft) {
                                //从第一个开始 第一个为null(不为第一章)
                                int i = 1;
                                for (BookPage bookPage : leftbookPageList) {
                                    bookPageList.add(i, bookPage);
                                    i++;
                                }
                            } else if (count == 1 && isLeft) {
                                //从第一个开始 第一个为null(为第一章)
                                int i = 0;
                                bookPageList.remove(0);
                                for (BookPage bookPage : leftbookPageList) {
                                    bookPageList.add(i, bookPage);
                                    i++;
                                }
                            ...
                            if (isLeft && count == 1) {
                                viewPagerBody.setCurrentItem(leftbookPageList.size() - 1, false);
                            } else if (isLeft && count > 1) {
                                viewPagerBody.setCurrentItem(leftbookPageList.size(), false);
                            }
                        }
    ...
    private void pageShow() {
        ...
        if (bookPageList.get(0) == null && nowPage == 0) {
            viewPager_body.setCurrentItem(1, false);
        }

        pagerAdapter.setMyOnScrollListener(this);

        viewPager_body.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {

            @Override
            public void onPageScrolled(int i, float v, int i1) {
                if (i == 0 && i1 == 0 && count == 1 && isOn1) {
                    viewPager_body.setCount(1, 0);
                } else {
                    isOn1 = false;
                }
            }
            ...
            public void onPageSelected(int i) {
                if (bookPageList.get(i) != null) {
                    viewPager_body.setCount(bookPageList.get(i).getCount(), bookPageList.get(i).getPage());
                }
                if (i == 0 && count > 1) {
                    updatePre(count);
                }
    ...
    @Override
    public void updatePre(int count) {
        Log.e("success", count + "");
        isLeft = true;
        this.count = count - 1;
        MyThread myThread = new MyThread();
    ...
    @Override
    public void updateNex(int count) {
        isLeft = false;
        this.count = count + 1;
        MyThread myThread = new MyThread();
        myThread.start();
